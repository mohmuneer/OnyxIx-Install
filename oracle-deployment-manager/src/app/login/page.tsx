  ...
  return (
    <div dir="rtl" lang="ar" style={{ position: 'relative', minHeight: '100vh', width: '100%', overflow: 'hidden', background: '#020617' }}>
      ...
      <header style={{
        ...
        display: 'flex', alignItems: 'center', padding: '0 24px', direction: 'rtl',
        background: 'rgba(10,15,35,0.85)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)',
        borderBottom: '1px solid rgba(255,255,255,0.06)',
      }}>
        {config.logo?.logoUrl ? (
          <img src={config.logo.logoUrl} alt="Logo" style={{ width: '36px', height: '36px', borderRadius: '12px', objectFit: 'contain', flexShrink: 0 }} />
        ) : (
          <div style={{ width: '36px', height: '36px', borderRadius: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, background: 'linear-gradient(135deg, #2563EB, #3B82F6)', boxShadow: '0 4px 12px rgba(37,99,235,0.3)' }}>
            <Database style={{ width: '18px', height: '18px', color: 'white' }} />
          </div>
        )}
        <div style={{ minWidth: 0, marginRight: '12px', textAlign: 'right' }}>
          <h1 style={{ fontSize: '14px', fontWeight: 700, color: 'white', fontFamily: FONT, margin: 0, lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {config.logo?.systemName || 'Onyx IX'}
          </h1>
          <p style={{ fontSize: '10px', color: '#64748b', fontFamily: FONT, margin: 0, lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
